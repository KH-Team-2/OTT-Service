    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (contentType != null &&  contentType.toLowerCase().startsWith("multipart/")) {
			...
			try {
				...
				if(res) {
					jsResponse("이미지 변경성공", "user.do?command=userlist", response);
				}else {
					jsResponse("이미지 변경실패", "user.do?command=userlist", response);
				}
			}catch(Exception e){
				e.printStackTrace();
			}
			...
			if(res) {
				jsResponse("성공", "user.do?command=userlist", response);
			}else {
				jsResponse("실패", "user.do?command=userlist", response);
			}
		}
	}
